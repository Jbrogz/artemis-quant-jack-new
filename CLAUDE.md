...
Single-factor (**momentum only**) crypto long/short factor book, **all data from Artemis**, following the pre-registered methodology in `docs/specs/2026-05-30-artemis-momentum-design.md` strictly. Package `amom`. Repo: `github.com/Jbrogz/artemis-quant-jack-new` (public).
...
- **Push gotcha:** run `gh auth switch -u Jbrogz` before every `git push` (the keychain may serve a different github.com credential → plain push 404s).
